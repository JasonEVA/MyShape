...
// 拼接文件夹下文件路径
- (NSString *)getDocumentDirectoryWithFolderName:(NSString *)folderName fileName:(NSString *)fileName
{
    
    NSString *fullPath = [[self getDocumentSubfolderWithFolderName:folderName ? folderName :@""] stringByAppendingPathComponent:fileName ? fileName : @""];
    NSLog(@"-------------->%@", fullPath);
    return fullPath;
}
