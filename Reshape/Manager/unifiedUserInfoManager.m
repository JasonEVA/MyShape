...
static NSString *const kflowState = @"flowState";   //流量提醒开关状态
static NSString *const kautoCacheState = @"autoCacheState"; //自动缓存开关状态

static NSString *const kLevelArr = @"levelArr"; //等级信息
...
- (BOOL)loginStatus {
    if ([self getLoginResultData].token.length > 0) {
        if ([[self getLoginResultData].token isEqualToString:kDefaultToken]) {
            return NO;
        } else {
            return YES;
        }
...
- (void)removeUserInfo
{
    [_defaults setObject:kDefaultToken forKey:kToken];
...
- (void)saveSwitchStatusWithTag:(SwitchTag)tag open:(BOOL)open
{
    switch (tag) {
        case tag_flow:
        {
            [_defaults setBool:open forKey:kflowState];
        }
            break;
        case tag_autoCache:
        {
            [_defaults setBool:open forKey:kautoCacheState];
        }
            break;
        default:
            break;
...
- (BOOL)getSwitchStatusWithTag:(SwitchTag)tag
{
    ...
    switch (tag) {
        case tag_flow:
        {
           isOpen = [_defaults boolForKey:kflowState];
        }
            break;
        case tag_autoCache:
        {
            isOpen = [_defaults boolForKey:kautoCacheState];
        }
            break;
        default:
            break;
...
{
    [_defaults setObject:array forKey:kLevelArr];
}

//获取登记信息
- (NSArray *)getLevelInfo{
    NSArray *arr = [NSArray arrayWithArray:[_defaults objectForKey:kLevelArr]];
    return arr;
}
@end
